...
const createSession = async (req,res,next) => {
    try {
        ...
        if(!currentCourse){
            return next(new HttpError("This Session needs a Course to be created" , 400));
        }
        if(!name||!data||!description||!req.files){
            return next(new HttpError("Fill in all fields and choose Media data" , 400));
        }
        const sessionExists = await Session.findOne({name: name,instructor: req.user.name});
        // to use this in the session checker
        if(sessionExists) {
            return next(new HttpError('Try new Name for your Session ',400))
        }
        const {media} =  req.files;
        if(media.size > 20000000){
            return next(new HttpError('Media too big , file should be less than 20mb.', 400))
        }
        ...
        media.mv(path.join(__dirname,'..','uploads',newFilename),async(err)=>{
            if(err){
                ...
                if(!newSession){
                    return next(new HttpError('Session could not be created.', 400))
                }
